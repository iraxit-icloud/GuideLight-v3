...
// MARK: - Integration Extension for NavigationViewModel

extension NavigationViewModel {
    
    /// Describe the user's current location and speak it
    func announceCurrentLocation(position: simd_float3) {
        let description = LocationDescriptionService.shared.describeCurrentLocation(
            position: position,
            map: map,
            includeNavigationHints: true
        )
        
        if let description = description {
            VoiceGuide.shared.speak(description)
        } else {
            VoiceGuide.shared.speak("Welcome to your navigation session. I'm ready to guide you to your destination.")
        }
    }
    
    /// Get current room for other purposes
    func getCurrentRoom(position: simd_float3) -> Room? {
        return LocationDescriptionService.shared.identifyCurrentRoom(position: position, map: map)
    }
}
